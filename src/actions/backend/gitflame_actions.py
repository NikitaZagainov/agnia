...
from src.models.gitflame_params import (
    GetRepoInfoInputParams,
    GetRepoInfoOutputParams,
    GetIssueInputParams,
    GetIssueOutputParams,
    GetRepoIssuesInputParams,
    GetRepoIssuesOutputParams,
    GitFlameOwner,
    CreateIssueInputParams,
    CreateIssueOutputParams,
    EditIssueInputParams,
    EditIssueOutputParams,
    DeleteIssueInputParams,
    DeleteIssueOutputParams,
)
from src.actions.registry import register_action
from src.actions.user_messages.gitflame_messages import form_create_issue_result_message
...
@register_action(
    ...
    GetRepoInfoOutputParams,
    system_name="GitFlame",
    action_name="Get repo information",
)
def get_repo_info(
    authorization_data: dict, input_data: GetRepoInfoInputParams
) -> GetRepoInfoOutputParams:
...
@register_action(
    ...
    result_message_func=form_create_issue_result_message,
)
def create_issue(authorization_data: dict, input_data: CreateIssueInputParams):
    headers = _get_headers(authorization_data)

    # add params where "disposition" == "body"
    body_params = {"title": input_data.title, "body": input_data.body}
    ...
    if response.status_code != 201:
        raise ActionException(
            f"Failed to create issue. Status Code: {response.status_code}. Details: {response.text}"
        )
...
def get_issue(authorization_data: dict, input_data: GetIssueInputParams):
    ...
    if response.status_code != 200:
        raise ActionException(
            f"Failed to get issue. Status Code: {response.status_code}. Details: {response.text}"
        )
...
def delete_issue(authorization_data: dict, input_data: DeleteIssueInputParams):
    ...
    if response.status_code != 204:
        raise ActionException(
            f"Failed to delete issue. Status Code: {response.status_code}. Details: {response.text}"
        )
...
def edit_issue(authorization_data: dict, input_data: EditIssueInputParams):
    ...
    if response.status_code != 201:
        raise ActionException(
            f"Failed to edit issue. Status Code: {response.status_code}. Details: {response.text}"
        )
...
def get_repo_issues(authorization_data: dict, input_data: GetRepoIssuesInputParams):
    ...
    if response.status_code != 200:
        raise ActionException(
            f"Failed to get issues. Status Code: {response.status_code}. Details: {response.text}"
        )
    ...
    for issue in response:
        ...
        output.append(GetIssueOutputParams(**issue_output))
