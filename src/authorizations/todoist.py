...
def callback(code: str = None, state: str = None, error: str = None):
    ...
    try:
        ...
        # client_id or client_secret parameters are incorrect:
            ...
            raise HTTPException(status_code=401, detail="Incorrect application credentials")
        else:
            raise HTTPException(status_code=500, detail="Token exchange failed")
    except requests.RequestException as req_err:
        raise HTTPException(status_code=500, detail=f"Failed to communicate with Todoist: {str(req_err)}")
