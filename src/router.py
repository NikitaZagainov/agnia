...
async def execute_action(
        system_name: str, action_name: str, input_data: dict,
        authorizations_data: Optional[dict] = None
) -> dict:
...
def form_result_message(system_name: str, action_name: str, execution_result: dict | None) -> dict:
    if execution_result is None:
        message_str = f"Could not run action '{action_name}' for system '{system_name}'"
        message_dict = {"Error": f"Could not run action '{action_name}' for system '{system_name}'"}
